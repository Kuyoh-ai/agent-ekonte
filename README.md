...
- **開発モードで 502 が出る**: `npm run dev` では画面（Vite, 5173）が API サーバー（8787）より先に起動します。画面は起動を待って自動で再接続するので、上部の「サーバーの起動を待っています…」が消えるまで待ってください。30 秒ほどで消えない場合は、ターミナルの `[server]` の行にエラーが出ています。
